...
AGING_THRESHOLD = 5     # Tempo de espera para tentar promover a prioridade (aging)

## Gerencia o escalonamento dos processos e o uso da CPU
class Scheduler:
    def __init__(self, mem: MemoryManager, res: ResourceManager) -> None:   #Inicializa escalonador
        ...
        ## Fila de admissão
        self.admission: Deque[Process] = deque()    # Fila de processos novos
    ...
    ## Adiciona um processo recém-chegado à fila de admissão
    def add_new_process(self, proc: Process) -> None:   
        self.admission.append(proc)

    ## Tenta admitir os processos presentes na fila de admissão: se houver memória e recursos disponíveis, o processo é colocado na fila de prontos; caso contrário, permanece aguardando
    def try_admit(self) -> None: 
        pending = deque()                               # Nova fila contendo apenas os processos que continuarão esperando
        while self.admission:                           # Processa os processos da fila de admissão
    ...
    ## Tenta alocar memória e recursos para um processo
    def _allocate_resources(self, proc: Process) -> bool:   
        page_table = self.memory.admit_process(         # Admite o processo na memória e cria a tabela de páginas
    ...
    def _enqueue_ready(self, proc: Process) -> None:    
        if proc.is_real_time:                           # Processos de tempo real utilizam uma fila FIFO exclusiva
            ...
            level = proc.current_queue_level - 1        # Converte a prioridade (1, 2, 3) para o índice da lista (0, 1 ou 2)
            self.user_queues[level].append(proc)

    ## Seleciona o próximo processo que utilizará a CPU
    def dispatch(self) -> Optional[Process]:            
        if self.current and self.current.state == ProcessState.RUNNING:     # Se já existe um processo executando, ele continua utilizando a CPU
            return self.current
        # Se CPU vazia, seleciona o próximo processo
        if self.rt_queue:
    ...
    ## Simula 1 ms de execução da CPU e realiza as operações de escalonamento
    def tick(self) -> None: 
        ...
        for _, q in enumerate(self.user_queues):
            for _ in range(len(q)):                     # Percorre todos os processos da fila atual
                ...
                if proc.waited_time >= AGING_THRESHOLD and proc.current_queue_level > 1:
                    proc.current_queue_level -= 1       # Promove processo
        ...
        if proc.is_real_time:  
            # Processos de tempo real possuem prioridade fixa e não sofrem preempção                     
    ...
    ## Libera os recursos utilizados quando um processo termina
    def _terminate_process(self, proc: Process) -> None: 
        ...
        # Libera os quadros de memória ocupados pelo processo
        self.memory.release_process(proc.pid) 
        # Libera os recursos de E/S utilizados pelo processo de usuário
        if not proc.is_real_time:                       
            self.resources.free(proc.printers, proc.scanners, proc.modems, proc.sata)


    @staticmethod
    ## Exibe as informações do processo despachado para execução
